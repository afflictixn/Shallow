...
public class MNISTDoubleLayer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataSetIterator mnistTrain = new MnistDataSetIterator(10000, true, rngSeed);
        DataSetIterator mnistTest = new MnistDataSetIterator(500, false, rngSeed);

        Model model = new Model();
        model
                .addLayer(new Linear(new LinearLayerConfig().units(50).weightInitializer(WeightInitEnum.HeNormal)))
                .addLayer(new ReLU())
                .addLayer(new Linear(new LinearLayerConfig().units(10).weightInitializer(WeightInitEnum.HeNormal)))
                .setLoss(new CategoricalCrossEntropyLoss())
                .setOptimizer(new Adam())
                .setScheduler(new IntervalBasedScheduler(20));
        DataSet dataSet = mnistTrain.next();
        model.fit(dataSet.getFeatures(), dataSet.getLabels(), 0.09, 64, 50);
        DataSet test = mnistTest.next();
