...
/**
 * Simple plotting methods for the MLPClassifier
 */
public class PlotUtil {
     ...
     */
    public static void plotTrainingData(Model model, DataSetIterator trainIter, INDArray backgroundIn, int nDivisions) {
        double[] mins = backgroundIn.min(0).data().asDouble();
        double[] maxs = backgroundIn.max(0).data().asDouble();

        DataSet ds = allBatches(trainIter);
        INDArray backgroundOut = model.predict(backgroundIn);
     ...
     * @param nDivisions    Number of points (per axis, for the backgroundIn/backgroundOut arrays)
     */
    public static void plotTestData(Model model, DataSetIterator testIter, INDArray backgroundIn, int nDivisions) {
        ...
        INDArray predicted = model.predict(ds.getFeatures());
