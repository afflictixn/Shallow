    ...
    public void init(List<BaseLayer> layers) {
        ...
        for(BaseLayer layer : layers){
            if(layer instanceof WeightedLayer weighted) {
                ...
                waGrads.get(waGrads.size() - 1).add(Nd4j.zerosLike(weighted.getWeightGrads()));
                waGrads.get(waGrads.size() - 1).add(Nd4j.zerosLike(weighted.getBiasGrads()));
                waSqGrads.get(waSqGrads.size() - 1).add(Nd4j.zerosLike(weighted.getWeightGrads()));
                waSqGrads.get(waSqGrads.size() - 1).add(Nd4j.zerosLike(weighted.getBiasGrads()));
            }
        }
    }
    public Adam() {
